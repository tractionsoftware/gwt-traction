...
public final class RgbaColor {
    ...
    /**
     * The GWT version of this class provides a default, but we want
     * to know about parsing failures so we default to null.
     *
     * @return null
     */
    private static RgbaColor getDefaultColor() {
        return null; // new RgbaColor(255,255,255);
    }
    ...
    private static String getRgbParts(String str) {
        Matcher m = rgb.matcher(str);
        return m.matches() ? m.replaceFirst("$1,$2,$3") : "";
    }

    private static String getRgbaParts(String str) {
        Matcher m = rgba.matcher(str);
        return m.matches() ? m.replaceFirst("$1,$2,$3,$4") : "";
    }

    private static String getHslParts(String str) {
        Matcher m = hsl.matcher(str);
        return m.matches() ? m.replaceFirst("$1,$2,$3") : "";
    }

    private static String getHslaParts(String str) {
        Matcher m = hsla.matcher(str);
        return m.matches() ? m.replaceFirst("$1,$2,$3,$4") : "";
    }

    private static int parseInt(String val, int radix) {
        try {
